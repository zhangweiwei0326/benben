...
?>
		...
		<div class="main_right_content_title">
			...
			<div class="main_right_content_content_block_action_add">
				<?php if($role & 1){?>
				<a class="btn btn-success" href="<?php echo Yii::app()->createUrl("user/create?add=add");?>">增加</a>
				<?php }?>
			</div>
		</div>
			...
			<table cellspacing=1 border="0" class="table table-hover">
				<thead>
					<tr class="main_right_content_content_title">
						<td width="20%">用户名</td><td width="20%">用户角色</td><td width="20%">创建时间</td><td width="20%">最近登录时间</td><td width="20%">操作</td>
					</tr>
				</thead>
				<tbody>
					<?php					$i=0;
	...
	<input id="dis_baseurl" type="hidden" value="<?php echo Yii::app()->createUrl("user/disable",array('page'=>$pages->currentPage +1));?>" />
	<div class="main_footer <?php //if($pages->pageCount <= 1) echo 'main_footer_hidden';?>">
		<div class="main_footer_page">
		<?php 
				$path = substr(dirname(__FILE__), 0, -4)."layouts/searchPages.php";
				require_once($path);  
			?>
			<?php
