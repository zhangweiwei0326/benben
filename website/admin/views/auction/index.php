...
<?php if($result['msg']) {?>
            ...
            <?php }?>
            <form action="<?php echo Yii::app()->createUrl('auction/index')?>" method="get">
            <ul style="margin-top:0;">
                <li>
                    ...
                    <div class="row" style="width:100%;">
                        <div class="col-md-4">
                            <label style="float: left;margin-left:12px;width:100px;margin-top: 7px;">状　　态:</label>
                        </div>
                        
                        <div class="col-md-8" style="float: left;margin:0 0 10px 28px;margin-left:-350px;width: 150px;">
    ...
    <table class="table table-bordered table-condensed table-hover definewidth m10" >
        ...
        <tbody>
            ...
            foreach ( $items as $item ) {
                $edit_url = Yii::app()->createUrl('auction/edit',array('auction_id'=>$item->auction_id));
                if($item -> is_close == "0"){
            ...
            <tr class="main_right_content_content_body">
                <td><input type="checkbox" class="selectOne" name="<?php echo $item->auction_id; ?>" /><?php echo $item->auction_id; ?></td>
...
<script type="text/javascript">
    ...
    $(".close-auction").on("click",function(){
        var _this=this;
        url="<?php echo Yii::app()->createUrl("auction/closeAuction")?>";//json输出
    ...
    $(".open-auction").on("click",function(){
        var _this=this;
        url="<?php echo Yii::app()->createUrl("auction/openAuction")?>";//json输出
        $.post(url, {auction_id:_this.name}, function(data){
            if (data.status==1) {
                $(_this).after('<a class="btn btn-primary btn-sm" href='+data.url+'>编辑</a>');
                $(_this).css({
