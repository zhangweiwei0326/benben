...
class BxapplyController extends BaseController
{
    ...
    public function actionIndex()
    {
        ...
        //非东阳百姓网不能拿到全百姓网信息
        $whereBx = "";
        if ($this->dongyang != $this->ownbx) {
            $cri->addSearchCondition('t.enterprise_id', $this->ownbx, true, 'AND');
            $whereBx = "type=3 and id=" . $this->ownbx;
        } else {
            $whereBx = "type=3";
        }

        //查询具体百姓网段
        $enterpriseAll = Enterprise::model()->findAll($whereBx);
        $enterprise = array();
        if ($enterpriseAll) {
            foreach ($enterpriseAll as $v) {
                $enterprise[] = array(
                    "id" => $v['id'],
                    "name" => $v['name']
                );
        ...
        if (isset($_GET) && !empty($_GET)) {
            ...
            if ($_GET['enterprise'] && ($_GET['enterprise'] != -1)) {
                $cri->addCondition('t.enterprise_id = ' . $_GET['enterprise'], 'AND');

                $result['enterprise_id'] = $_GET['enterprise'];
                $result['goback'] = -2;
            }
        }
        ...
        $this->render('index', array('items' => $items, 'pages' => $pages, 'result' => $result, 'msg' => $msg,
            'province' => $province, 'res' => $res, 'res2' => $res2, 'enterprise' => $enterprise));
