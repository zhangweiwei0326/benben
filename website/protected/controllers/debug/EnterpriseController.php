...
class EnterpriseController extends PublicController
{
	...
	public function actionMyenterprisein(){		
		...
		foreach ($enterprisein as $val){
 		...
 		$enterpriseall = array();
 		if($eid){
 			$sql = "select id,name,member_id,number,type,status,created_time  from enterprise where id in (".implode(",", $eid).") order by created_time desc ";//and status = 0
	...
	public function actionSearch(){
		...
		if($keyword){
			$sql = "select id,name,member_id,number,type,created_time  from enterprise where name like '%{$keyword}%' and status = 0 order by created_time desc limit 50";
		}else{
			$sql = "select id,name,member_id,number,type,created_time  from enterprise where status = 0 order by created_time desc limit 50";
		}
		$command = $connection->createCommand($sql);
		$enterprise = $command->queryAll();
		//我加入的政企通讯录
...
// 		}
	...
	public function actionMember(){
		...
		$logResult = EnterpriseDisplayMemberLog::model()->find("enterprise_id = {$enterpriseid} and member_id = {$user->id}");
		
		$displayList = EnterpriseDisplayMember::model()->findAll(array('select'=>'*', 'condition'=>"enterprise_id = {$enterpriseid} and user_id = {$user->id}"));
		if (empty($logResult)) {
			//虚拟通讯录
			if($enterpriseType == 2){
				if($origin == 1){
					...
					if (count($enterpriseMemberList) > 0) {						
						foreach ($enterpriseMemberList as $key => $value) {
							if ($value['short_phone']) {
								$enterpriseMemberInfo[$value['short_phone']] = array('id'=>$value['id'], 'member_id'=>$value['member_id'], 'short_phone'=>$this->eraseNull($value['short_phone']), 'phone'=>($enterpriseType == 2) ? "":$this->eraseNull($value['phone']), 'name'=>$value['name']);
							}
						}					
					}
				...
				if($origin == 2){
					//查看当前通讯录中的所有用户
					$enterpriseMemberList = EnterpriseMember::model()->findAll(array('select'=>'*', 'condition'=>"contact_id = {$enterpriseid} and phone <> '{$my->phone}'"));
						
					$enterpriseMemberInfo = array();
					if (count($enterpriseMemberList) > 0) {
						foreach ($enterpriseMemberList as $key => $value) {
							if ($value['phone']) {
								$enterpriseMemberInfo[$value['phone']] = array('id'=>$value['id'], 'member_id'=>$value['member_id'], 'short_phone'=>$this->eraseNull($value['short_phone']), 'phone'=>($enterpriseType == 2) ? "":$this->eraseNull($value['phone']), 'name'=>$value['name']);
							}
						}
					}
				...
				if (count($enterpriseMemberList) > 0) {
					...
					foreach ($enterpriseMemberList as $key => $value) {
						if ($value['member_id']) {
							...
							$enterpriseMemberInfo[$value['member_id']] = array('id'=>$value['id'], 'member_id'=>$value['member_id'], 'short_phone'=>$this->eraseNull($value['short_phone']),  'phone'=>$this->eraseNull($value['phone']), 'name'=>$value['name']);
						}else{
							$enterpriseMemberInfo[$value['phone']] = array('id'=>$value['id'], 'member_id'=>$value['member_id'], 'short_phone'=>$this->eraseNull($value['short_phone']), 'phone'=>$this->eraseNull($value['phone']), 'name'=>$value['name']);
						}
					}
					if (count($benbenId) > 0) {
						...
						foreach ($benbenMember as $key => $value) {
							$enterpriseMemberInfo[$value['phone']] = array('id'=>$enterpriseMemberInfo[$value['id']]['id'], 'member_id'=>$value['id'], 'short_phone'=>$this->eraseNull($enterpriseMemberInfo[$value['id']]['short_phone']), 'phone'=>($enterpriseType == 2) ? "":$this->eraseNull($enterpriseMemberInfo[$value['id']]['phone']), 'name'=>$enterpriseMemberInfo[$value['id']]['name']);
			...
			if (count($displayList) > 0	) {
				...
				if (count($enterpriseMemberList) > 0) {
					foreach ($enterpriseMemberList as $key => $value) {
						$enterpriseMemberInfo[$value['id']] = array('id'=>$value['id'], 'member_id'=>$value['member_id'], 'short_phone'=>$this->eraseNull($value['short_phone']), 'phone'=>$this->eraseNull($value['phone']), 'name'=>$value['name']);
					}	
				}
			}
					...
					if (isset($displayListInfo[$value['id']])) {
						...
						foreach($existMember as $each){
							...
							if (isset($enterpriseMemberInfo[$eachId])) {
								...
								$group_member[] = array('id'=>$currentInfo['id'], 'member_id'=>$currentInfo['member_id'], 'short_phone'=>$currentInfo['short_phone'],'phone'=>$currentInfo['phone'], 'name'=>$currentInfo['name']);
							}
						}
			...
			if (isset($displayListInfo[0])) {
				...
				$eachId = $each['member_id'];
					if (isset($enterpriseMemberInfo[$eachId])) {
						$currentInfo = $enterpriseMemberInfo[$eachId];
						$group_member[] = array('id'=>$currentInfo['id'], 'member_id'=>$currentInfo['member_id'], 'short_phone'=>$currentInfo['short_phone'], 'phone'=>($enterpriseType == 2) ? "":$currentInfo['phone'], 'name'=>$currentInfo['name']);
					}
				}
			...
			$result['common_count'] = count($displayList);
	...
	public function actionMemberSearch(){
		...
		if (count($commonId) > 0) {
			...
			$displayList = EnterpriseDisplayMember::model()->findAll(array('select'=>'member_id', 'condition'=>"enterprise_id = {$enterpriseid} and user_id = {$user->id} and member_id in (".implode(",", $commonId).")"));
			if(count($displayList) > 0){
		...
		for ($i=0; $i < count($result); $i++) { 
			...
			$returnArray[] = array('id'=>$result[$i]['id'], 'member_id'=>$result[$i]['member_id'], 'short_phone'=>$this->eraseNull($result[$i]['short_phone']),'phone'=>($enterpriseType == 2) ? "":$this->eraseNull($result[$i]['phone']), 'name'=>$result[$i]['name'], 'common'=>$common );
		}
		$return['ret_num'] = 0;
		$return['ret_msg'] = '操作成功';
	...
	public function actionEgroup(){
		...
		$connection = Yii::app()->db;
		if($my){
			$sqla = "select id,groupname,created_time from enterprise_group where member_id = {$my->id} order by id asc";
			$command = $connection->createCommand($sqla);
			$result0 = $command->queryAll();
		}
		
		//查出各分组里的总人数
		$sql = "select group_id, count(*) as c from enterprise_display_member where enterprise_id = {$enterpriseid} and user_id = ".$user->id." group by group_id";
		$command = $connection->createCommand($sql);
...
// 		$shortphone = Frame::getStringFromRequest('short_phone');
		...
		foreach ($contactinfo as $valu){			
			...
			if(!in_array($valu['phone'], $inenenterprise) && !in_array($valu['short_phone'], $inenenterprise)){						
				...
				$t = time();
				$notphone[] = "({$enterpriseid},'{$valu['phone']}','{$valu['short_phone']}',{$memberid},'{$valu['name']}',{$t}, $user->id)";
				$num++;
			}
			//添加常用联系人，不超过50人
			if ($display_count < 50) {
		...
		if(count($notphone) > 0){
			...
			for ($i=0; $i < count($notphone); $i++) { 
				$value = $notphone[$i];
				$sql2 = "insert into enterprise_member (contact_id,phone,short_phone,member_id,name,created_time, invite_id) values ".$value;
				$command = $connection->createCommand($sql2);
				$result2 = $command->execute();
...
// 		}
		...
		if($contactinfoArray[0]){
			...
			foreach ($contactinfo as $valu){
				...
				if(!in_array($valu['short_phone'], $inenenterprise)){
					$memberid = 0;
					$t = time();
					$notphone[] = "({$enterpriseid},'','{$valu['short_phone']}',{$memberid},'{$valu['name']}',{$t}, $user->id)";
					$num++;
				}
				//添加常用联系人，不超过50人
				if ($display_count < 50) {
	...
	public function actionCommon(){
		...
		if(count($allMember) > 0){
			$displayList = EnterpriseDisplayMember::model()->findAll(array('select'=>'member_id', 'condition'=>"enterprise_id = {$enterpriseid} and user_id = {$user->id} and member_id in (".implode(',', $allMember).")"));
			if (count($displayList) > 0) {
				$result['ret_num'] = 1008;
	...
	public function actionDeletegroup(){
		...
		if ($groupid>0) {
			...
			$sql = "delete from  enterprise_display_member  where group_id = 0 and enterprise_id = {$enterpriseid}";
		}
	...
	*/
	public function actionInviteList()
	{
		...
		$enterpriseType = $enterprise['type'];
		$sql = "select * from enterprise_member where contact_id = ".$enterpriseid." and invite_id = ".$user->id;
		$command = $connection->createCommand($sql);
