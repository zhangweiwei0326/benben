	...
	public function actionList(){
		...
		if($creationid){
			...
			foreach($benben_arr as $kb=>$vb){
				$memberinfo=Member::model()->find("benben_id={$vb}");
				$memberid[]=$memberinfo['id'];
				$benben=$this->getfriend($user['id'],2);
				$membername[$memberinfo['id']]=$benben[$memberinfo['benben_id']];
			}
			...
			inner join member b on a.member_id = b.id where a.circle_id in ({$creationid}) and a.replier in (".implode(",",$memberid).") order by a.created_time asc";
			$command = $connection->createCommand($sql2);
			$result2 = $command->queryAll();
			$comment = array();
			foreach($result2 as $key => $va){
