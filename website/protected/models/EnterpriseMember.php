...
class EnterpriseMember extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('contact_id, member_id, created_time,sort,firstin', 'numerical', 'integerOnly'=>true),
			array('short_phone, phone', 'length', 'max'=>20),
			array('remark_name,pinyin,allpinyin', 'length', 'max'=>255),
			array('name', 'length', 'max'=>11),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, contact_id, member_id, short_phone, remark_name, created_time, phone, name,sort,firstin,pinyin,allpinyin', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('firstin',$this->firstin);
